package usecase

import (
	"context"

	"monorepo/services/library-service/internal/modules/reservation/domain"

	"github.com/golangid/candi/tracer"
)

func (uc *reservationUsecaseImpl) CreateReservation(ctx context.Context, req *domain.RequestReservation) (result domain.ResponseReservation, err error) {
	trace, ctx := tracer.StartTraceWithContext(ctx, "ReservationUsecase:CreateReservation")
	defer trace.Finish()

	data := req.Deserialize()
	err = <- uc.repoSQL.ReservationRepo().Save(ctx, &data)
	result.Serialize(&data)

	// Sample using broker publisher
	// uc.deps.GetBroker(types.Kafka). // get registered broker type (sample Kafka)
	// 				GetPublisher().
	// 				PublishMessage(ctx, &candishared.PublisherArgument{
	// 		Topic:   "[topic]",
	// 		Key:     "[key]",
	// 		Message: candihelper.ToBytes("[message]"),
	// 	})
	return
}
